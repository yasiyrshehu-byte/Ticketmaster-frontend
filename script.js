...
function showHelp(){alert("Need help? Use For You to edit your event and tickets.")}
function toast(text){
...
function eventCard(g){
  return '<article class="event-card"><img src="'+esc(img(g.image))+'" alt=""><div class="shade"></div>'+
    ...
    '<div class="event-title">'+esc(g.eventName)+'</div><div class="event-venue">'+esc(g.venue)+(g.location?" • "+esc(g.location):"")+'</div></div>'+
    '<button class="view-button" onclick="go(\'/event/'+encodeURIComponent(g.id)+'\')"><span class="small-icon">'+ticketGlyph()+'</span>View Tickets</button></article>';
}
...
function eventPage(id){
  ...
  return render(
    ...
    '<main class="content">'+
      ...
      '</div>'+
      '<div class="ticket-tabs"><div class="ticket-tab active">Tickets</div><div class="ticket-tab">Extras</div></div>'+
      '<section class="order">'+
        '<div class="order-head"><div><div class="order-num">Order #'+esc(g.order)+'</div><div class="order-sub">x'+count+' Ticket'+(count===1?"":"s")+'</div></div>'+
        '<button class="dots" onclick="eventMenu(\''+encodeURIComponent(g.id)+'\')">⋮</button></div>'+
        ...
        '<div class="map-actions"><button onclick="toast(\'Upgrade options\')">↥ Upgrade</button>'+
      ...
      '</section>'+
      '<section class="extras"><h2>EXTRAS</h2>'+
...
function viewTickets(id){
  ...
  const m=el("modal");
  const cards=g.tickets.map(t=>
    '<div class="entry-card"><div class="entry-blue"><div class="entry-name">'+esc(g.eventName)+'</div>'+
    ...
    '<div class="barcode"></div><div class="barcode-number">'+esc(t.barcode||g.order||"")+'</div>'+
    '<button class="wallet" onclick="toast(\'Wallet action ready to connect\')">Add to Wallet</button><div class="qr-note">Entry code appears only in View Tickets.</div></div>'
  ).join("");
...
function eventMenu(id){
  const g=state.tickets.find(x=>x.id===decodeURIComponent(id));if(!g)return;
  const m=el("modal");
  m.innerHTML='<div class="sheet"><div class="sheet-handle"></div><div class="sheet-head"><span>Event Options</span><button class="close" onclick="closeModal()">Close</button></div>'+
    '<div class="sheet-body"><button class="primary" onclick="editEvent(\''+encodeURIComponent(g.id)+'\');closeModal()">Edit Event</button>'+
...
function cancelTransfer(id,i){
  const d=decodeURIComponent(id);
  state.transfers=state.transfers.filter(x=>!(x.eventId===d&&x.ticketIndex===i&&x.status==="pending"));save();eventPage(d);
}
...
function discover(){
  ...
  const box=el("discoverSearch");if(box)box.addEventListener("keydown",e=>{if(e.key==="Enter")toast("Search ready")});
}
