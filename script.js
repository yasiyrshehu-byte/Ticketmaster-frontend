...
const KEY="ticketwaves_state_v11";
const LEGACY_KEYS=["ticketwaves_state_v10","ticketwaves_state_v9","ticketwaves_state_v8","ticketwaves_state_v7","ticketwaves_state_v6"];
...
const defaultState={
  ...
  tickets:[{
    ...
    tickets:[
      {id:"seat-1",section:"N110",row:"24",seat:"14",barcode:"000000000001"}
    ]
  }],
  transfers:[],
...
function render(body,active){el("app").innerHTML='<div class="app"><div class="shell">'+body+'</div>'+bottom(active)+'</div>'}
function showHelp(){alert("Use For You to create or edit events and tickets.")}
function toast(text){
...
function myTickets(){
  ...
  render(header("My Events",false)+
    ...
    '<div class="event-list">'+(upcoming.length?upcoming.map(eventCard).join(""):'<div class="empty"><h2>No upcoming events</h2><p>Add one from For You.</p></div>')+'</div>',"/my-tickets")
}
function showPast(){
  ...
  render(header("My Events",false)+
    ...
    '<div class="event-list">'+(past.length?past.map(eventCard).join(""):'<div class="empty"><h2>No past events</h2><p>Your past events will appear here.</p></div>')+'</div>',"/my-tickets")
}
function eventCard(g){
  return '<article class="event-card"><img src="'+esc(img(g.image))+'" alt=""><div class="shade"></div>'+
    '<div class="event-copy"><div class="event-date">'+esc(dateLabel(g.date,g.time))+'</div><div class="event-title">'+esc(g.eventName)+'</div>'+
    '<div class="event-venue">'+esc(g.venue)+(g.location?" • "+esc(g.location):"")+'</div></div>'+
    '<button class="view-button" onclick="go(\'/event/'+encodeURIComponent(g.id)+'\')"><span class="small-icon">▣</span>View Tickets</button></article>'
}

/* ---- Ticket detail: sharp stepped/angled card, no emoji ticket ---- */
  ...
  return render(
    '<main class="ticket-screen">'+
      '<div class="ticket-hero">'+
        '<img class="ticket-hero-image" src="'+esc(img(g.image))+'" alt="">'+
        '<button class="hero-back" onclick="history.back()" aria-label="Back">‹</button>'+
        '<button class="hero-help" onclick="eventMenu(\''+encodeURIComponent(g.id)+'\')" aria-label="Options">•••</button>'+
      '</div>'+
      '<section class="ticket-stack">'+
        '<div class="ticket-info-panel">'+
          '<div class="ticket-date">'+esc(dateLabel(g.date,g.time))+'</div>'+
          '<div class="ticket-event-title">'+esc(g.eventName)+'</div>'+
          '<div class="ticket-venue-row"><span>'+esc(g.venue)+(g.location?" - "+esc(g.location):"")+'</span>'+
            '<span class="ticket-count"><span class="stack-icon" aria-hidden="true"></span>x'+count+'</span></div>'+
        '</div>'+
        '<button class="barcode-cta" onclick="viewTickets(\''+encodeURIComponent(g.id)+'\')">'+
          '<span class="barcode-icon" aria-hidden="true"><i></i><i></i><i></i><i></i><i></i><i></i></span>'+
          '<span>View Tickets</span>'+
        '</button>'+
      '</section>'+
      '<section class="ticket-detail-tabs">'+
        '<button class="detail-tab active">Tickets</button><button class="detail-tab">Extras</button>'+
      '</section>'+
      '<section class="ticket-order">'+
        '<div class="order-head"><div><div class="order-num">Order #'+esc(g.order)+'</div><div class="order-sub">x'+count+' Ticket'+(count===1?"":"s")+'</div></div>'+
        '<button class="order-dots" onclick="eventMenu(\''+encodeURIComponent(g.id)+'\')">⋮</button></div>'+
        ...
        '<div class="map-actions"><button onclick="toast(\'Upgrade options\')">↥ Upgrade</button><button onclick="openTransfer(\''+encodeURIComponent(g.id)+'\')">↗ Transfer</button><button onclick="startSell(\''+encodeURIComponent(g.id)+'\')">⟳ Sell</button></div>'+
      ...
      '</section>'+
      '<section class="extras"><h2>EXTRAS</h2><div class="got-card"><div class="got-visual"><img src="'+esc(img(g.image))+'" alt=""><div class="got-word">YOU GOT<br>TICKETS!</div></div><div class="share"><h3>Post on Social Media</h3><p>Build hype for the event, and share that you got tickets with your friends and family.</p><button onclick="shareEvent(\''+encodeURIComponent(g.id)+'\')">Share You’re Going ↗</button></div></div></section>'+
    '</main>',"/my-tickets")
}

function ticketCard(g,t,i){
  const p=state.transfers.find(x=>x.eventId===g.id&&x.ticketIndex===i&&x.status==="pending");
  return '<div class="ticket-card"><div class="ticket-head">'+esc(g.extraInfo||"MOBILE TICKET")+'</div>'+
...
/* Entry screen: blue barcode/show screen, multiple tickets, swipe arrows */
function viewTickets(id){
  const g=state.tickets.find(x=>x.id===decodeURIComponent(id));if(!g)return;
  const m=el("modal");
  let index=0;
  function draw(){
    const t=g.tickets[index];
    m.innerHTML='<div class="entry-screen">'+
      '<div class="entry-top"><button class="entry-back" onclick="closeModal()">‹</button><div class="entry-title">View Ticket</div><button class="entry-more" onclick="eventMenu(\''+encodeURIComponent(g.id)+'\')">•••</button></div>'+
      '<div class="entry-art"><img src="'+esc(img(g.image))+'" alt=""></div>'+
      '<div class="entry-event"><div class="entry-event-name">'+esc(g.eventName)+'</div><div class="entry-event-date">'+esc(dateLabel(g.date,g.time))+'</div><div class="entry-event-venue">'+esc(g.venue)+(g.location?" - "+esc(g.location):"")+'</div></div>'+
      '<div class="entry-ticket-fields"><div><small>SECTION</small><strong>'+esc(t.section||"—")+'</strong></div><div><small>ROW</small><strong>'+esc(t.row||"—")+'</strong></div><div><small>SEAT</small><strong>'+esc(t.seat||"—")+'</strong></div></div>'+
      '<div class="entry-code-note">SCHOOL DEMO • LIVE-STYLE MOVING CODE</div>'+
      '<div class="live-barcode"><span class="scan-line"></span></div>'+
      '<div class="barcode-id">'+esc(t.barcode||g.order||"")+'</div>'+
      '<div class="entry-count">'+(index+1)+' of '+g.tickets.length+'</div>'+
      '<div class="entry-nav">'+(index>0?'<button onclick="window.__entryPrev()">‹</button>':'<span></span>')+(index<g.tickets.length-1?'<button onclick="window.__entryNext()">›</button>':'<span></span>')+'</div>'+
      '<button class="entry-wallet" onclick="toast(\'Wallet action ready to connect\')">Add to Wallet</button>'+
      '<div class="entry-bottom-actions"><button onclick="closeModal()">Done</button><button onclick="openTransfer(\''+encodeURIComponent(g.id)+'\')">Transfer</button><button onclick="startSell(\''+encodeURIComponent(g.id)+'\')">Sell</button></div>'+
    '</div>';
  }
  window.__entryPrev=()=>{if(index>0){index--;draw()}};
  window.__entryNext=()=>{if(index<g.tickets.length-1){index++;draw()}};
...
function deleteEvent(id){
  const d=decodeURIComponent(id);if(!confirm("Delete this event?"))return;
...
function transferReady(){
  const ok=el("trFirst")?.value.trim()&&el("trLast")?.value.trim()&&el("trEmail")?.value.trim();el("forwardBtn")?.classList.toggle("ready",!!ok)
}
...
function cancelTransfer(id,i){
  const d=decodeURIComponent(id);state.transfers=state.transfers.filter(x=>!(x.eventId===d&&x.ticketIndex===i&&x.status==="pending"));save();go("/event/"+encodeURIComponent(d))
}
function startSell(id){const g=state.tickets.find(x=>x.id===decodeURIComponent(id));if(g)toast("Sell flow ready for connection")}
function discover(){
  render(header("Discover",false)+'<main class="content"><div class="discover-hero"><input class="search" id="discoverSearch" placeholder="Artist, Event or Venue"></div><div class="list-card"><h3>Find your next event</h3><p>Browse events and use For You to create or edit your ticket details.</p></div></main>',"/discover");
}
function forYou(editId){
  ...
  render(header(editing?"Edit Event":"For You",true)+'<form class="form" onsubmit="saveEvent(event,\''+(editing?encodeURIComponent(editing.id):"")+'\')">'+
    ...
    '<div class="field"><label>Event Image</label><input class="file-input" type="file" accept="image/*" onchange="previewImage(event)"><input id="fImage" class="input" placeholder="Or paste image URL" value="'+(g.image&&g.image.startsWith("data:")?"":esc(g.image))+'"><div id="imagePreview"></div></div>'+
...
function saveEvent(e,id){
  ...
  if(id){const d=decodeURIComponent(id),i=state.tickets.findIndex(x=>x.id===d);if(i>=0)state.tickets[i]=Object.assign({id:d},data)}
